...
struct CrossOverBottle: Identifiable, Hashable, Sendable {
    let name: String
    let url: URL
    let version: String
    let is64Bit: Bool
...
enum CrossOverIntegrationError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .unsupportedExecutable: String(localized: "windowsGames.error.unsupportedExecutable")
        case .gameIsRunning: String(localized: "windowsGames.error.gameRunning")
        case .missingProxy(let name): String(localized: "windowsGames.error.missingProxy \(name)")
        case .commandFailed(let detail): String(localized: "windowsGames.error.commandFailed \(detail)")
        case .unsafeRestore(let name): String(localized: "windowsGames.error.changedFile \(name)")
        }
    }
}
