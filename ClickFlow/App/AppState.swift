...
final class AppState: ObservableObject {
    ...
    init() {
        ...
        permissionManager.onPostingPermissionLost = { [weak self] in
            ...
            self.userMessage = String(localized: "error.accessibilityRequired")
            self.emergencyStop()
        }
    ...
    func startClicker() {
        ...
        guard permissionManager.canPostEvents else {
            ...
            userMessage = String(localized: "error.accessibilityRequired")
            return
        }
        Task { @MainActor [weak self] in
            guard let self else { return }
    ...
    func startRecording(continuingMacroID: UUID? = nil) {
        ...
        guard permissionManager.canListenToEvents else {
            ...
            userMessage = String(localized: "error.inputMonitoringRequired")
            return
        }
    ...
    func finishRecording(
        ...
        var macro = MouseMacro(
            name: String(localized: "macro.defaultName \(macros.count + 1)"),
    ...
    func newMacro() {
        let macro = MouseMacro(name: String(localized: "macro.defaultName \(macros.count + 1)"))
    ...
    func duplicateSelectedMacro() {
        ...
        copy.id = UUID()
        copy.name = String(localized: "macro.copyName \(copy.name)")
        copy.createdAt = .now
        copy.updatedAt = .now
    ...
    func playRecentMacro() {
        ...
        guard let macro else {
            userMessage = String(localized: "error.noMacro")
            return
        }
        play(macro: macro)
    }
    ...
    func loadMacros() async {
        ...
        if !result.damagedFileNames.isEmpty {
            userMessage = String(localized: "error.damagedMacros \(result.damagedFileNames.joined(separator: ", "))")
        }
    }
    ...
    func finishCombinedRecording(
        ...
        let macro = CombinedMacro(
            name: String(localized: "combined.defaultName \(combinedMacros.count + 1)"),
            events: captured
        )
    ...
    func newCombinedMacro() {
        let macro = CombinedMacro(name: String(localized: "combined.defaultName \(combinedMacros.count + 1)"))
    ...
    func playRecentCombinedMacro() {
        ...
        guard let macro else {
            userMessage = String(localized: "error.noCombinedMacro")
            return
        }
    ...
    func loadCombinedMacros() async {
        ...
        if !result.damagedFileNames.isEmpty {
            userMessage = String(localized: "error.damagedCombinedMacros \(result.damagedFileNames.joined(separator: ", "))")
        }
    }
    ...
    private func play(combinedMacro: CombinedMacro) {
        ...
        guard !combinedMacro.events.isEmpty else {
            userMessage = String(localized: "error.emptyMacro")
            return
        }
        ...
        guard invisible.isEmpty else {
            ...
            userMessage = String(localized: "error.offscreenEvents \(rows)")
            return
        }
        ...
        Task { @MainActor [weak self] in
            ...
            if combinedMacro.containsControllerEvents,
               settingsStore.shouldShowControllerPlaybackNotice {
                ...
                userMessage = String(localized: "combined.controller.playbackLimitation")
            }
        }
    }
    ...
    private func play(macro: MouseMacro) {
        ...
        guard invisibleIndices.isEmpty else {
            ...
            userMessage = String(localized: "error.offscreenEvents \(displayIndices)")
            return
        }
