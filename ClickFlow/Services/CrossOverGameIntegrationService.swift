...
actor CrossOverGameIntegrationService {
    ...
    init(
        ...
        self.proxyRootURL = proxyRootURL ?? Bundle.main.resourceURL
        self.recordsRootURL = recordsRootURL ?? fileManager.urls(
    ...
    func plan(gameURL: URL, preset requestedPreset: CrossOverGamePreset) throws -> CrossOverAdapterPlan {
        guard fileManager.fileExists(atPath: gameURL.path),
            ...
            throw CrossOverIntegrationError.invalidSelection(
                String(localized: "windowsGames.error.chooseExecutable")
            )
        }
        ...
        if let fixed = resolvedPreset.fixedDLLNames {
            ...
            guard !detected.isEmpty else {
                throw CrossOverIntegrationError.invalidSelection(
                    String(localized: "windowsGames.error.noXInputEvidence")
                )
            }
    ...
    func diagnose(
        ...
        guard let gameURL else {
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.game"),
                detail: String(localized: "windowsGames.error.chooseExecutable")
            ))
        ...
        do {
            ...
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.plan"),
                detail: plan.dllNames.joined(separator: ", ")
            ))
            if !plan.antiCheatFiles.isEmpty {
                items.append(.init(
                    id: "anticheat", severity: .warning,
                    title: String(localized: "windowsGames.check.antiCheat"),
        ...
        if let bottle, fileManager.fileExists(atPath: bottle.registryURL.path) {
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.bottle"),
            ...
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.bottle"),
                detail: String(localized: "windowsGames.error.chooseBottle")
            ))
        }
        ...
        if let application, fileManager.isExecutableFile(atPath: application.wineURL.path) {
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.crossOver"),
            ...
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.crossOver"),
                detail: String(localized: "windowsGames.error.chooseCrossOver")
            ))
        }
        ...
        if let resolvedPlan, let directory = resolvedPlan.architecture.resourceDirectoryName,
           resolvedPlan.dllNames.allSatisfy({ proxyURL(named: $0, architectureDirectory: directory) != nil }) {
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.resources"),
                detail: String(localized: "windowsGames.check.resources.ready")
            ))
        } else {
            items.append(.init(
                id: "resources", severity: .failed,
                title: String(localized: "windowsGames.check.resources"),
                detail: String(localized: "windowsGames.check.resources.missing")
            ))
        }
        ...
        items.append(.init(
            ...
            title: String(localized: "windowsGames.check.process"),
            detail: String(localized: running ? "windowsGames.check.process.running" : "windowsGames.check.process.stopped")
        ))
        ...
        items.append(.init(
            ...
            title: String(localized: "windowsGames.check.publisher"),
        ...
        if let installed {
            items.append(.init(
                ...
                title: String(localized: "windowsGames.check.installation"),
                detail: String(localized: installed.state == .installed
               ...
               let bottle, let application, let resolvedPlan {
                ...
                for file in installed.files {
                    ...
                    items.append(.init(
                        ...
                        title: file.name,
                        detail: String(localized: intact
                ...
                for dll in resolvedPlan.dllNames {
                    ...
                    items.append(.init(
                        ...
                        title: dll,
                        detail: value ?? String(localized: "windowsGames.check.registry.missing")
                    ))
                }
            }
        }
    ...
    func install(
        ...
        guard fileManager.fileExists(atPath: bottle.registryURL.path) else {
            throw CrossOverIntegrationError.invalidSelection(String(localized: "windowsGames.error.chooseBottle"))
        }
        guard fileManager.isExecutableFile(atPath: application.wineURL.path) else {
            throw CrossOverIntegrationError.invalidSelection(String(localized: "windowsGames.error.chooseCrossOver"))
        }
    ...
    private func discoverBottles() -> [CrossOverBottle] {
        ...
        return children.compactMap { url in
            ...
            return CrossOverBottle(name: url.lastPathComponent, url: url, version: version, is64Bit: is64Bit)
        }
    ...
    private func proxyURL(named name: String, architectureDirectory: String) -> URL? {
        ...
        let url = root.appendingPathComponent("\(architectureDirectory)-\(name)")
    ...
    private func controllerHeartbeatDescription() -> (active: Bool, detail: String) {
        ...
        guard let attributes = try? fileManager.attributesOfItem(atPath: url.path),
            ...
            return (false, String(localized: "windowsGames.check.publisher.inactive"))
        }
