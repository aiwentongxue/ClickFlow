...
struct HotkeySettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("hotkey.captureHint")
                .font(.footnote)
                .foregroundStyle(.secondary)

            hotkeyGroup("hotkey.group.clicker", actions: HotkeyAction.clickerCases)
            hotkeyGroup("hotkey.group.mouseMacro", actions: HotkeyAction.mouseMacroCases)
            hotkeyGroup("hotkey.group.combinedMacro", actions: HotkeyAction.combinedMacroCases)

            Button("hotkey.clearAll") {
    ...
    private func hotkeyGroup(_ titleKey: LocalizedStringKey, actions: [HotkeyAction]) -> some View {
        GroupBox {
            VStack(spacing: 10) {
...
private final class KeyCaptureButton: NSButton {
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        isCapturing = true
        onCaptureBegan?()
        title = String(localized: "hotkey.pressNow")
    ...
    func updateTitle() {
        guard !isCapturing else { return }
        title = configuration?.displayString ?? String(localized: "hotkey.notSet")
    }
