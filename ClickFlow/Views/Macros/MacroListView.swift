...
struct MacroListView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            if let selectedID = appState.selectedMacroID,
               let selectedMacro = appState.macros.first(where: { $0.id == selectedID }) {
                ...
                ContentUnavailableView("macros.select", systemImage: "tablecells")
                    .frame(minWidth: 350, maxWidth: .infinity, maxHeight: .infinity)
            }
        }
        ...
        .toolbar {
            ...
            Button(appState.isRecording ? "macros.stopRecording" : "macros.record", systemImage: appState.isRecording ? "stop.fill" : "record.circle") {
                appState.toggleRecording(
                    stoppedByMouseControl: appState.isRecording && ControlActivation.isMouseTriggered
                )
            }
            Button("macros.duplicate", systemImage: "plus.square.on.square") { appState.duplicateSelectedMacro() }
            ...
            Button("macros.delete", systemImage: "trash") { showingDeleteConfirmation = true }
                .disabled(appState.selectedMacroID == nil || appState.isRecording)
            Button(appState.isPlayingMacro ? "macros.stop" : "macros.play", systemImage: appState.isPlayingMacro ? "stop.fill" : "play.fill") {
                if appState.isPlayingMacro { appState.stopPlayback() } else { appState.playSelectedMacro() }
            }
                .disabled(appState.selectedMacroID == nil || appState.isRecording)
            Button(
                appState.isMouseMacroPaused ? "macros.resumePlayback" : "macros.pause",
        ...
        .confirmationDialog("macros.delete.confirm", isPresented: $showingDeleteConfirmation) {
            Button("macros.delete", role: .destructive) { appState.deleteSelectedMacro() }
        }
        .safeAreaInset(edge: .bottom) {
            if appState.isRecording {
                HStack {
                    ...
                    Text("macros.recordingEvents \(appState.recordingEventCount)")
                    Spacer()
                ...
                HStack {
                    ...
                    if let total = appState.playbackLoopCount {
                        ...
                        Text("macros.playingLoop \(appState.playbackLoop)")
                    }
                    Spacer()
                    Button(appState.isMouseMacroPaused ? "macros.resumePlayback" : "macros.pause") {
                        appState.toggleMouseMacroPause()
                    }
                    Button("macros.stop") { appState.stopPlayback() }
                }
                .padding(10)
                .background(.bar)
            }
