...
final class CrossOverGameSetupViewModel: ObservableObject {
    ...
    @Published var bottles: [CrossOverBottle] = []
    ...
    var selectedBottle: CrossOverBottle? {
    ...
    func install() async {
        guard let gameURL, let bottle = selectedBottle, let application = selectedApplication else {
            statusMessage = String(localized: "windowsGames.error.incompleteSelection")
            return
        }
        isBusy = true
        do {
            ...
            statusMessage = String(localized: "windowsGames.install.success")
        } catch {
    ...
    func restore() async {
        ...
        do {
            ...
            statusMessage = String(localized: "windowsGames.restore.success")
        } catch {
            statusMessage = error.localizedDescription
        }
        report = await service.diagnose(
    ...
    func copyReport() {
        ...
        statusMessage = String(localized: "windowsGames.diagnostics.copied")
    }
...
struct CrossOverGameSetupView: View {
    ...
    var body: some View {
        ...
        .fileImporter(
            ...
            guard url.pathExtension.lowercased() == "exe" else {
                model.statusMessage = String(localized: "windowsGames.error.chooseExecutable")
                return
            }
            Task { await model.selectGame(url) }
        }
        .alert("app.message.title", isPresented: statusBinding) {
            Button("common.ok") { model.statusMessage = nil }
        } message: {
            Text(model.statusMessage ?? "")
        }
        .confirmationDialog(
            "windowsGames.install.confirm.title",
            isPresented: $showInstallConfirmation,
            titleVisibility: .visible
        ) {
            Button("windowsGames.install.action") { Task { await model.install() } }
            Button("common.cancel", role: .cancel) {}
        } message: {
            Text(installConfirmationMessage)
        }
        .confirmationDialog(
            "windowsGames.restore.confirm.title",
            isPresented: $showRestoreConfirmation,
            titleVisibility: .visible
        ) {
            Button("windowsGames.restore.action", role: .destructive) { Task { await model.restore() } }
            Button("common.cancel", role: .cancel) {}
        } message: {
            Text("windowsGames.restore.confirm.message")
        }
    }
    ...
    private var introduction: some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 8) {
                Label("windowsGames.title", systemImage: "gamecontroller.fill")
                    .font(.title2.bold())
                Text("windowsGames.explanation")
                    .foregroundStyle(.secondary)
                Label("windowsGames.notVirtualHID", systemImage: "info.circle")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                Label("windowsGames.antiCheatWarning", systemImage: "exclamationmark.shield")
                    .font(.footnote)
    ...
    private var configuration: some View {
        GroupBox("windowsGames.configuration") {
            Grid(alignment: .leading, horizontalSpacing: 14, verticalSpacing: 12) {
                GridRow {
                    Text("windowsGames.game")
                    HStack {
                        Text(model.gameURL?.path ?? String(localized: "windowsGames.game.none"))
                            .lineLimit(1)
                            .truncationMode(.middle)
                            .foregroundStyle(model.gameURL == nil ? .secondary : .primary)
                        Spacer()
                        Button("windowsGames.game.choose") { isChoosingGame = true }
                    }
                }
                Divider().gridCellColumns(2)
                GridRow {
                    Text("windowsGames.bottle")
                    Picker("windowsGames.bottle", selection: $model.selectedBottleID) {
                        Text("windowsGames.selection.none").tag(String?.none)
                ...
                GridRow {
                    Text("windowsGames.crossOver")
                    Picker("windowsGames.crossOver", selection: $model.selectedApplicationID) {
                        Text("windowsGames.selection.none").tag(String?.none)
                ...
                GridRow {
                    Text("windowsGames.profile")
                    Picker("windowsGames.profile", selection: $model.preset) {
                        ForEach(CrossOverGamePreset.allCases) { preset in
                            Text(preset.localizedKey).tag(preset)
                        }
                    }
                    .labelsHidden()
                }
    ...
    private var actions: some View {
        HStack(spacing: 10) {
            Button {
                ...
                Label("windowsGames.refresh", systemImage: "arrow.clockwise")
            }
            Button {
                Task { await model.diagnose() }
            } label: {
                Label("windowsGames.diagnose", systemImage: "stethoscope")
            }
            ...
            if model.installedManifest != nil {
                Button(role: .destructive) { showRestoreConfirmation = true } label: {
                    Label("windowsGames.restore.action", systemImage: "arrow.uturn.backward.circle")
                }
                .disabled(model.isBusy)
            } else {
                Button { showInstallConfirmation = true } label: {
                    Label("windowsGames.install.action", systemImage: "shippingbox.and.arrow.backward")
                }
                .buttonStyle(.borderedProminent)
    ...
    private var diagnostics: some View {
        if let report = model.report {
            GroupBox("windowsGames.diagnostics") {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    HStack {
                        Spacer()
                        Button("windowsGames.diagnostics.copy") { model.copyReport() }
                    }
                    .padding(.top, 10)
                }
            }
            ...
            ContentUnavailableView(
                "windowsGames.diagnostics.empty.title",
                systemImage: "stethoscope",
                description: Text("windowsGames.diagnostics.empty.message")
            )
    ...
    private var installConfirmationMessage: String {
        if model.antiCheatFiles.isEmpty {
            return String(localized: "windowsGames.install.confirm.message")
        }
        return String(localized: "windowsGames.install.confirm.antiCheat \(model.antiCheatFiles.joined(separator: ", "))")
    }
}
